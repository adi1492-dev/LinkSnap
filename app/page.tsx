        ...
        <nav className="px-4 py-2 space-y-1 flex-1">
          ...
          <div className="mt-8">
            <div className="p-4 bg-gray-100 rounded-xl border border-gray-200">
              ...
              <a 
                href={isMounted ? "javascript:(function(){window.open('" + window.location.origin + "?url='+encodeURIComponent(window.location.href));})();" : "#"}
                className="mt-2 block w-full py-2 px-3 bg-white border border-gray-300 rounded-md text-xs font-medium text-center shadow-sm hover:bg-gray-50 transition-colors"
        ...
        {currentUser && (
          <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between gap-2">
            <div className="truncate min-w-0">
              ...
              <p className="font-mono text-[9px] text-gray-400 truncate mt-0.5">{currentUser.email}</p>
            </div>
            <button 
              onClick={() => logoutUser()} 
              title="Sign Out"
...
function HomeTab() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-gray-900 mb-2">Build a Preview</h2>
        <p className="text-gray-500">Paste any valid URL to extract and visualize its Open Graph metadata.</p>
      </div>
      ...
      <form onSubmit={handlePreview} className="relative w-full max-w-3xl">
        ...
        <input
          ...
          className="block w-full pl-11 pr-32 py-4 text-base border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-all outline-none border focus:ring-2"
          required
        />
      ...
      {error && (
        <div className="p-4 bg-red-50 text-red-700 rounded-lg max-w-3xl border border-red-100">
      ...
      {data && (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 pt-8">
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-gray-50/50 rounded-2xl p-8 border border-dashed border-gray-300 flex justify-center items-center" style={{ backgroundImage: 'radial-gradient(#e5e7eb 1px, transparent 1px)', backgroundSize: '16px 16px' }}>
              <PreviewCard data={data} url={url} />
            </div>

            <h3 className="text-lg font-semibold border-b pb-2 pt-6">Detailed Metadata Overview</h3>
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden text-sm shadow-sm">
              <table className="w-full text-left">
                <tbody className="divide-y divide-gray-100">
                  {Object.entries(data).filter(([, v]) => v != null && v !== '' && (!Array.isArray(v) || v.length > 0)).map(([key, value]) => (
                    <tr key={key} className="hover:bg-gray-50 transition-colors">
                      <td className="py-3 px-4 font-medium text-gray-500 w-1/4 capitalize align-top whitespace-nowrap">
                        {key.replace(/([A-Z])/g, ' $1').trim()}
                      </td>
                      <td className="py-3 px-4 text-gray-900 break-all align-top">
          ...
          <div className="lg:col-span-2 space-y-6">
             ...
             <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
               <div className="px-4 py-2 border-b border-gray-800 bg-gray-900/50 flex justify-between items-center">
                 <span className="text-xs font-mono text-gray-400">HTML Iframe</span>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-6">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 mb-2">History & Collections</h2>
          <p className="text-gray-500">View and organize all your previously generated previews.</p>
        </div>
        <div className="relative w-full sm:w-72">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm outline-none transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex gap-2 pb-2 overflow-x-auto">
        <button 
          ...
          className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${!activeCollection ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        >
          All
        </button>
        {collections.map(col => (
          <button 
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${activeCollection === col ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
            {col}
          </button>
        ))}
      ...
      {filteredHistory.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
          ...
          <p className="text-gray-500 mt-1">Generate some previews to see them appear here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredHistory.map(item => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow group relative flex flex-col">
              {item.data.image ? (
                ...
                <div className="h-40 bg-gray-100 flex items-center justify-center">
                  <Globe className="w-8 h-8 opacity-20 text-gray-900" />
                </div>
              )}
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <h4 className="font-semibold text-gray-900 text-sm line-clamp-2 leading-snug mb-1">
                ...
                <div className="mt-auto pt-4 flex items-center gap-2 flex-wrap text-xs">
                  ...
                  {/* Tags */}
                  {item.tags.map(tag => (
                     <span key={tag} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 text-gray-600 rounded-md">
              ...
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  ...
                  className="bg-white/90 p-1.5 rounded-md text-red-600 hover:bg-red-50 shadow-sm"
                >
...
function ApiDashboardTab() {
    ...
    return (
      <div className="max-w-md mx-auto my-12 animate-in fade-in slide-in-from-bottom-6 duration-500">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Register or sign in to generate secure API keys, monitor developer request logs, and access developer metrics.</p>
        </div>

        <div className="bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
          <div className="grid grid-cols-2 border-b border-gray-100 bg-gray-50 text-sm font-medium">
            <button
              onClick={() => { setIsRegistering(false); setAuthError(null); }}
              className={`py-3.5 text-center transition-colors ${!isRegistering ? 'bg-white text-gray-900 border-b-2 border-blue-600 font-semibold' : 'text-gray-500 hover:text-gray-800'}`}
            >
              Sign In
            </button>
            <button
          ...
          <form onSubmit={handleAuthSubmit} className="p-6 space-y-4">
            {authError && (
              <div className="p-3 bg-red-50 text-red-700 text-xs rounded-lg border border-red-100 leading-relaxed font-medium">
                {authError}
              </div>
            )}
            ...
            {isRegistering && (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-500 block uppercase tracking-wider">Full Name</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    required
                  />
                </div>
              </div>
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-gray-500 block uppercase tracking-wider">Email Address</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all/css"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-gray-500 block uppercase tracking-wider">Password</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full pl-9 pr-10 py-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                  required
                />
                <button
                  type="button"
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-6">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 mb-2">API Keys & Controls</h2>
          <p className="text-gray-500">Acquire endpoints, check authentication secrets, and trace interactive calls.</p>
        </div>
        <div className="flex items-center gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 shrink-0">
          <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-semibold text-sm uppercase">
            {currentUser.name[0]}
          </div>
          <div>
            <p className="text-xs font-semibold text-blue-900 leading-none">{currentUser.name}</p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2.5 text-gray-500 mb-3">
              ...
              <h3 className="font-semibold text-gray-900 text-sm tracking-tight">API Request Quota</h3>
            </div>
            <div className="flex items-baseline gap-1.5 mt-2">
              <span className="text-4xl font-extrabold text-gray-900">{totalRequests}</span>
              <span className="text-gray-400 font-medium">/ {currentUser.quotaLimit} queries</span>
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2.5 text-gray-500 mb-3">
              ...
              <h3 className="font-semibold text-gray-900 text-sm tracking-tight">Active Access Keys</h3>
            </div>
            <p className="text-4xl font-extrabold text-gray-900 mt-2">{userKeys.length}</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2.5 text-gray-500 mb-3">
              ...
              <h3 className="font-semibold text-gray-900 text-sm tracking-tight">System Global Limits</h3>
            </div>
            <p className="text-2xl font-extrabold text-gray-900 mt-2">60 requests</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex justify-between items-center">
          <h3 className="font-semibold text-gray-900 text-base">Secret Key Management</h3>
          <span className="text-xs bg-gray-100 text-gray-600 py-1 px-2.5 rounded-full font-medium">Require Bearer Auth</span>
        </div>
        ...
        <div className="p-6">
          <form onSubmit={handleCreateKey} className="flex gap-3 mb-6">
            <input 
              ...
              className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm outline-none transition-all"
              required
            />
            <button 
              type="submit"
              className="px-5 py-2.5 bg-gray-900 hover:bg-gray-800 text-white font-semibold rounded-lg transition-colors text-sm shrink-0 flex items-center gap-1.5"
            >
          ...
          {userKeys.length === 0 ? (
            <div className="text-center py-10 border border-dashed border-gray-200 rounded-xl">
              ...
              <p className="text-gray-500 text-sm">No active API keys generated. Issue a key above to configure your clients.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead>
                  <tr className="text-gray-400 border-b border-gray-100 text-xs font-semibold uppercase tracking-wider">
                  ...
                  {userKeys.map(k => {
                    ...
                    return (
                      <tr key={k.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="py-4 font-semibold text-gray-900">{k.name}</td>
                        <td className="py-4">
                          <div className="flex items-center gap-2">
                            <code className="px-2.5 py-1 bg-gray-100 text-gray-800 rounded font-mono text-xs max-w-xs block truncate">
                            ...
                            <button
                              ...
                              className="text-gray-400 hover:text-blue-600 p-1 rounded transition-colors relative"
                              title="Copy Token"
                            >
                        ...
                        <td className="py-4 text-gray-500 text-xs">{format(k.createdAt, 'MMM d, yyyy')}</td>
                        <td className="py-4 text-center font-semibold text-gray-700">{k.requestsCount}</td>
                        <td className="py-4 text-right">
                          <button 
                            onClick={() => deleteApiKey(k.id)}
                            className="bg-transparent hover:bg-red-50 hover:text-red-600 text-gray-400 p-2 rounded-lg transition-colors ml-auto flex items-center justify-center"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
          <div className="border-b pb-3.5">
            <h3 className="font-semibold text-gray-900 text-base">API Playground (Live Client Scraper)</h3>
            <p className="text-xs text-gray-400 mt-1">Simulate real GET requests to the client metadata endpoints with your active API key secrets.</p>
          </div>

          <form onSubmit={triggerPlaygroundRequest} className="space-y-4 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1 sm:col-span-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Target Client Key</label>
                <select
              ...
              <div className="space-y-1 sm:col-span-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Url Parameter Target</label>
          ...
          {playgroundError && (
            <div className="p-3 bg-red-50 text-red-700 text-xs rounded-lg border border-red-100 font-medium leading-relaxed mt-2 animate-in fade-in">
              {playgroundError}
            </div>
          )}
          ...
          {(playgroundLatency !== null || playgroundStatusCode !== null) && (
            <div className="flex gap-3 text-xs border rounded-lg p-2.5 bg-gray-50 text-gray-500 font-mono animate-in fade-in">
          ...
          {playgroundResult && (
            <div className="space-y-2 mt-4 animate-in fade-in">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Response Meta Object (JSON)</span>
        ...
        <div className="lg:col-span-2 bg-gray-950 rounded-2xl border border-gray-900 shadow-xl flex flex-col overflow-hidden max-h-[420px]">
          <div className="px-4 py-3 border-b border-gray-900 bg-gray-950 text-gray-400 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse" />
              <span className="text-xs font-bold font-mono tracking-wider uppercase text-gray-300">Live API Request Stream</span>
            </div>
            {userLogs.length > 0 && (
              <button 
                onClick={clearLogs}
                className="text-[10px] bg-gray-900 hover:bg-gray-800 border border-gray-800 hover:text-white transition-colors py-1 px-2 rounded text-gray-400"
              >
                Clear Stream
              </button>
            )}
          ...
          <div className="p-4 flex-1 overflow-y-auto font-mono text-[10px] text-gray-300 space-y-2.5 leading-relaxed bg-[#0b0f19]">
            {userLogs.length === 0 ? (
              <div className="text-center text-gray-600 py-16 flex flex-col items-center justify-center space-y-2">
                <Terminal className="w-8 h-8 opacity-25" />
                <p>Waiting for incoming requests...</p>
                <p className="text-[9px] text-gray-700">Submit requests in the playground to stream logs live</p>
              </div>
            ) : (
              userLogs.map(l => (
                <div key={l.id} className="border-b border-gray-900/50 pb-2 flex flex-col gap-1.5 animate-in fade-in slide-in-from-top-1">
                  <div className="flex justify-between text-gray-400 text-[9px] items-center">
                    <span>{format(l.timestamp, 'HH:mm:ss.SSS')}</span>
                    <span className="px-1 py-0.5 rounded bg-gray-900/80 text-gray-500 font-semibold">{l.keyName}</span>
                  </div>
                  <div className="flex items-center gap-1 bg-gray-900/20 py-1 px-1.5 rounded text-white overflow-hidden text-ellipsis whitespace-nowrap">
                    ...
                    <span className="text-gray-400 truncate shrink min-w-0" title={l.url}>{l.url}</span>
                  </div>
                  <div className="flex gap-2.5 text-gray-500 text-[9px]">
                    <span>
                      STATUS:{' '}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="border-b border-gray-200 bg-gray-50">
          <div className="flex border-b border-gray-200 overflow-x-auto">
            <button
              ...
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${docsTab === 'quick' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              Quick Start
            </button>
            <button
              onClick={() => setDocsTab('endpoints')}
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${docsTab === 'endpoints' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              Endpoints & Params
            </button>
            <button
              onClick={() => setDocsTab('snippets')}
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${docsTab === 'snippets' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              Code Snippets
            </button>
            <button
              onClick={() => setDocsTab('errors')}
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${docsTab === 'errors' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              Error Standard
            </button>
          </div>
        ...
        <div className="p-6">
          {docsTab === 'quick' && (
            <div className="space-y-4 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div className="border border-gray-150 p-4 rounded-xl space-y-1.5 bg-gray-50/50">
                  <div className="font-semibold text-xs text-blue-600 tracking-wider uppercase font-semibold">Step 1</div>
                  <h5 className="font-bold text-sm text-gray-900">Issue Secret Access Key</h5>
                ...
                <div className="border border-gray-150 p-4 rounded-xl space-y-1.5 bg-gray-50/50">
                  <div className="font-semibold text-xs text-blue-600 tracking-wider uppercase font-semibold">Step 2</div>
                  <h5 className="font-bold text-sm text-gray-900">Configure Web Authorization</h5>
                  <p className="text-xs text-gray-500 leading-normal">
                    Securely append your key with HTTP requests using the header: <br />
                    <code className="font-mono text-[10px] font-bold bg-gray-100 px-1 py-0.5 rounded text-gray-700 mt-1 block">Authorization: Bearer pk_...</code>
                  </p>
                </div>

                <div className="border border-gray-150 p-4 rounded-xl space-y-1.5 bg-gray-50/50">
                  <div className="font-semibold text-xs text-blue-600 tracking-wider uppercase font-semibold">Step 3</div>
                  <h5 className="font-bold text-sm text-gray-900">Parse Extracted JSON Response</h5>
                  <p className="text-xs text-gray-500 leading-normal">
          ...
          {docsTab === 'endpoints' && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex gap-2 items-center">
                <span className="font-mono text-xs font-semibold px-2 py-0.5 bg-green-100 text-green-700 rounded-md">GET</span>
                <code className="font-mono text-xs font-semibold text-gray-900">/api/preview</code>
              </div>
              <p className="text-sm text-gray-500 leading-relaxed">
              ...
              <div className="overflow-x-auto border border-gray-200 rounded-xl mt-3">
                <table className="w-full text-left text-xs text-gray-500">
                  <thead className="bg-gray-50 text-gray-700 font-semibold uppercase text-[10px] tracking-wider border-b border-gray-200">
                    <tr>
                  ...
                  <tbody className="divide-y divide-gray-100 font-sans">
                    <tr>
                      <td className="p-3 font-mono font-semibold text-gray-900">url</td>
                      <td className="p-3 text-gray-400 font-medium">string</td>
                      <td className="p-3 text-red-600 font-semibold">YES</td>
                      <td className="p-3 text-gray-500">The url-encoded webpage URL you want to scrape metadata for. Fully supported schemes comprise both http and https.</td>
                    </tr>
                    <tr>
                      <td className="p-3 font-mono font-semibold text-gray-900">api_key</td>
                      <td className="p-3 text-gray-400 font-medium">string</td>
                      <td className="p-3 font-medium">No (Header alternative)</td>
                      <td className="p-3 text-gray-500">Your secret API key. Passed alternatively in the URL query string as <code className="bg-gray-100 p-0.5 rounded font-mono text-[10px]">?api_key=pk_...</code>. The HTTP Bearer Header is heavily preferred.</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          {docsTab === 'snippets' && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex justify-between items-center bg-gray-50 p-2 border border-gray-200 rounded-xl">
                <div className="flex gap-1.5 overflow-x-auto">
                  {(['curl', 'js', 'node', 'python'] as const).map(lang => (
                    <button
                      key={lang}
                      onClick={() => setCodeLang(lang)}
                      className={`text-xs px-3 py-1.5 font-semibold rounded-md transition-colors ${codeLang === lang ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-200/50'}`}
                    >
                ...
                <button 
                  ...
                  className="text-gray-500 hover:text-gray-900 py-1.5 px-3 rounded-md hover:bg-gray-200/50 text-xs font-semibold flex items-center gap-1 shrink-0"
                >
              ...
              <div className="bg-gray-950 rounded-2xl border border-gray-900 overflow-hidden shadow-inner pt-4">
                <div className="p-4 overflow-x-auto text-xs font-mono leading-relaxed text-gray-300 select-all max-h-72">
                  <pre>{getDocCode()}</pre>
                </div>
              </div>
            </div>
          ...
          {docsTab === 'errors' && (
            <div className="space-y-4 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border border-red-100 p-4 rounded-xl bg-red-50/20 space-y-1">
                  <div className="font-mono text-xs text-red-600 font-bold">400 Bad Request / Invalid URL</div>
                  <p className="text-xs text-gray-500 leading-normal">
                    This triggers when the <code className="bg-gray-100 px-0.5 rounded text-[10px]">url</code> param is missing or fails formal format validation checks.
                  </p>
                </div>
                ...
                <div className="border border-gray-200 p-4 rounded-xl bg-gray-50/50 space-y-1">
                  <div className="font-mono text-xs text-gray-700 font-bold">500 Server Scrapping Error</div>
                  <p className="text-xs text-gray-500 leading-normal">
